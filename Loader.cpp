// https://gist.github.com/el2ro/8914557
#include <bfd.h>
#include <stdio.h>
#include <cstdlib>
#include <string>
...
static int print_section_content(bfd *abfd, const char *name);
...
// check if the file is executable and has valid format
...
static int load_symbols_bfd(bfd* bfd_h, Binary * bin){
 ...
 Symbol *sym;
 bfd_symtab = NULL;
 // how many bytes to allocate for symbol table
 ...
 if(n < 0) {
    ...
    // populates your symbol table and returns the number of symbols it placed in the table
 ...
 for(i = 0; i < nsyms; i++) {
 // if BSF_FUNCTION flag is set that indicates it is a function symbol
...
static int load_dynsym_bfd(bfd *bfd_h, Binary *bin){
 ...
 Symbol *sym;
 bfd_dynsym = NULL;
 // here you call 'dynamic' upper bound
...
static int load_sections_bfd(bfd* bfd_h, Binary *bin){
 ...
 for(bfd_sec = bfd_h->sections; bfd_sec; bfd_sec = bfd_sec->next) {
     ...
     // It’s possible that the section has no name, will return NULL
     secname = bfd_get_section_by_name(bfd_h, bfd_sec->name)->name;
     if(!secname)
        secname = "<unnamed>";
...
print_section_content(bfd_h, sec->name.c_str());
    }
...
static int print_section_content(bfd *abfd, const char *name){
    ...
    printf("section name %s \t section content %s\n", name, bfd_get_section_by_name(abfd, ".text"));
    return 0;
}
